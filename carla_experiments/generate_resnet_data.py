...
def something_task(world: carla.World, actors: ResnetActors) -> ResnetActors:
    return actors
...
def initialize_actors(world: carla.World) -> ResnetActors:
    ...
    def save_image_and_controls(image: carla.Image):
        ...
        with open(f"output/{timestampstr}/controls/{image.frame:6d}.json", "w") as f:
            json.dump(state, f)
        image.save_to_disk(f"output/{timestampstr}/images/{image.frame:6d}.jpg")
