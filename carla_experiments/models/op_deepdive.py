...
class SequencePlanningNetwork(nn.Module):
    def __init__(self, M, num_pts):
        ...
        self.gru = nn.GRU(
            input_size=1024, hidden_size=512, bidirectional=True, batch_first=True
        )  # 1024 out
    ...
    def forward(self, x, hidden):
        ...
        raw_preds = self.plan_head(features)
        raw_preds, hidden = self.gru(
            raw_preds[:, None, :], hidden
        )  # N, L, H_in for batch_first=True
...
class MultipleTrajectoryPredictionLoss(nn.Module):
    ...
    def forward(self, pred_cls, pred_trajectory, gt):
        ...
        with torch.no_grad():
            ...
            gt_end_positions = gt[:, self.num_pts - 1 :, :].expand(
                -1, self.M, -1
            )  # B, 1, 3 -> B, M, 3

            distances = 1 - self.distance_func(
                pred_end_positions, gt_end_positions
            )  # B, M
            index = distances.argmin(dim=1)  # B

        gt_cls = index
        pred_trajectory = pred_trajectory[
            torch.tensor(range(len(gt_cls)), device=gt_cls.device), index, ...
        ]  # B, num_pts, 3
...
# THE MAIN NETWORK OP-DEEPDIVE
class SequenceBaselineV1(nn.Module):
    def __init__(
        self, M, num_pts, mtp_alpha, lr, optimizer, optimize_per_n_step=40
    ) -> None:
        super().__init__()
        self.M = M
        self.num_pts = num_pts
    ...
    def configure_optimizers(args, model):
        if args.optimizer == "sgd":
            optimizer = optim.SGD(
                model.parameters(), lr=args.lr, momentum=0.9, weight_decay=0.01
            )
