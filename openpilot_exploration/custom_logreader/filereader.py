...
def atomic_write_in_dir(
    ...
    if not overwrite and os.path.exists(path):
        raise FileExistsError(
            f"File '{path}' already exists. To overwrite it, set 'overwrite' to True."
        )
...
class URLFile:
    ...
    def pool_manager() -> PoolManager:
        if URLFile._pool_manager is None:
            ...
            retries = Retry(
                total=5, backoff_factor=0.5, status_forcelist=[409, 429, 503, 504]
            )
    ...
    def _request(
        ...
        return URLFile.pool_manager().request(
    ...
    def get_length(self) -> int:
        ...
        file_length_path = os.path.join(
            Paths.download_cache_root(), hash_256(self._url) + "_length"
        )
    ...
    def read(self, ll: Optional[int] = None) -> bytes:
    ...
    def read_aux(self, ll: Optional[int] = None) -> bytes:
        ...
        if response_code == 416:  # Requested Range Not Satisfiable
            raise URLFileException(
                f"Error, range out of bounds {response_code} {headers} ({self._url}): {repr(ret)[:500]}"
            )
        if download_range and response_code != 206:  # Partial Content
            raise URLFileException(
                f"Error, requested range but got unexpected response {response_code} {headers} ({self._url}): {repr(ret)[:500]}"
            )
        if (not download_range) and response_code != 200:  # OK
            raise URLFileException(
                f"Error {response_code} {headers} ({self._url}): {repr(ret)[:500]}"
            )
