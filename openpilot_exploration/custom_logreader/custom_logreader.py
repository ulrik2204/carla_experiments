...
from openpilot_exploration.custom_logreader import log as capnp_log
from openpilot_exploration.custom_logreader.filereader import (
    FileReader,
    file_exists,
    internal_source_available,
)
from openpilot_exploration.custom_logreader.route import Route, SegmentRange

cloudlog = logging.getLogger(__name__)
LogMessage = capnp._DynamicStructReader
LogIterable = Iterable[LogMessage]
RawLogIterable = Iterable[bytes]


def get_comma_segments_url(route_name: str, seg: int) -> str:
    return "get_comma_segments_url"


def get_url(*args, **kwards):
    return "get_url"
...
class _LogFileReader:
    def __init__(
        ...
        try:
            for e in ents:
                _ents.append(e)
        except capnp.KjException:  # type: ignore
            warnings.warn("Corrupted events detected", RuntimeWarning, stacklevel=1)

        self._ents = list(
            sorted(_ents, key=lambda x: x.logMonoTime) if sort_by_time else _ents
        )
        ...
        for ent in self._ents:
            if self._only_union_types:
                try:
                    ent.which()
                    yield ent
                except capnp.lib.capnp.KjException:  # type: ignore
                    pass
            else:
                yield ent
...
class ReadMode(StrEnum):
    ...
    AUTO_INTERACTIVE = (
        "i"  # default to rlogs, fallback to qlogs with a prompt from the user
    )
...
def auto_strategy(
    ...
    qlog_paths: LogPaths,
    interactive: bool,
    valid_file: ValidFileCallable,
) -> LogPaths:
    # auto select logs based on availability
    if any(rlog is None or not valid_file(rlog) for rlog in rlog_paths) and all(
        qlog is not None and valid_file(qlog) for qlog in qlog_paths
    ):
        if interactive:
            if (
                input(
                    "Some rlogs were not found, would you like to fallback to qlogs for those segments? (y/n) "
                ).lower()
                != "y"
            ):
                return rlog_paths
            ...
            cloudlog.warning(
                "Some rlogs were not found, falling back to qlogs for those segments..."
            )

        return [
            rlog if valid_file(rlog) else (qlog if valid_file(qlog) else None)
...
def apply_strategy(
    ...
    rlog_paths: LogPaths,
    qlog_paths: LogPaths,
    valid_file: ValidFileCallable = default_valid_file,
) -> LogPaths:
...
def internal_source(sr: SegmentRange, mode: ReadMode) -> LogPaths:
    ...
    rlog_paths: LogPaths = [get_internal_url(sr, seg, "rlog") for seg in sr.seg_idxs]
    qlog_paths: LogPaths = [get_internal_url(sr, seg, "qlog") for seg in sr.seg_idxs]

    return apply_strategy(mode, rlog_paths, qlog_paths)


def openpilotci_source(sr: SegmentRange, mode: ReadMode) -> LogPaths:
    rlog_paths: LogPaths = [get_url(sr.route_name, seg, "rlog") for seg in sr.seg_idxs]
    qlog_paths: LogPaths = [get_url(sr.route_name, seg, "qlog") for seg in sr.seg_idxs]
...
def auto_source(sr: SegmentRange, mode=ReadMode.RLOG) -> LogPaths:
    ...
    raise Exception(
        f"auto_source could not find any valid source, exceptions for sources: {exceptions}"
    )
...
def parse_direct(identifier: str):
    if (
        identifier.startswith(("http://", "https://", "cd:/"))
        or pathlib.Path(identifier).exists()
    ):
        return identifier
    return None
...
class LogReader:
    def _parse_identifiers(self, identifier: Union[str, List[str]]):
        ...
        assert (
            invalid_count == 0
        ), f"{invalid_count}/{len(identifiers)} invalid log(s) found, please ensure all logs \
are uploaded or auto fallback to qlogs with '/a' selector at the end of the route name."
    ...
    def _get_lr(self, i):
        if i not in self.__lrs:
            self.__lrs[i] = _LogFileReader(
                self.logreader_identifiers[i],
    ...
    def run_across_segments(self, num_processes, func):
        with multiprocessing.Pool(num_processes) as pool:
            ...
            for p in tqdm.tqdm(
                pool.imap(partial(self._run_on_segment, func), range(num_segs)),
                total=num_segs,
            ):
                ret.extend(p)
            return ret
    ...
    def filter(self, msg_type: str):
        return (
            getattr(m, m.which()) for m in filter(lambda m: m.which() == msg_type, self)
        )
...
if __name__ == "__main__":
    ...
    print("some")
    lr = LogReader(
        "/home/ulrikro/datasets/CommaAI/2024_02_28_Orkdal/2024-02-28--13-00-06--10/rlog",
    )
    for msg in lr:
        print(msg)
        break
