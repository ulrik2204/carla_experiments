...
from openpilot_exploration.custom_logreader.api import CommaApi
from openpilot_exploration.custom_logreader.hw import Paths
...
class RE:
    ...
    INDEX = r"-?[0-9]+"
    SLICE = rf"(?P<start>{INDEX})?:?(?P<end>{INDEX})?:?(?P<step>{INDEX})?"
    SEGMENT_RANGE = (
        rf"{ROUTE_NAME}(?:(--|/)(?P<slice>({SLICE})))?(?:/(?P<selector>([qras])))?"
    )

    BOOTLOG_NAME = ROUTE_NAME

    EXPLORER_FILE = (
        rf"^(?P<segment_name>{SEGMENT_NAME})--(?P<file_name>[a-z]+\.[a-z0-9]+)$"
    )
    OP_SEGMENT_DIR = rf"^(?P<segment_name>{SEGMENT_NAME})$"
...
class Route:
    ...
    def log_paths(self):
        ...
        return [
            log_path_by_seg_num.get(i, None) for i in range(self.max_seg_number + 1)
        ]

    def qlog_paths(self):
        qlog_path_by_seg_num = {s.name.segment_num: s.qlog_path for s in self._segments}
        return [
            qlog_path_by_seg_num.get(i, None) for i in range(self.max_seg_number + 1)
        ]

    def camera_paths(self):
        camera_path_by_seg_num = {
            s.name.segment_num: s.camera_path for s in self._segments
        }
        return [
            camera_path_by_seg_num.get(i, None) for i in range(self.max_seg_number + 1)
        ]

    def dcamera_paths(self):
        dcamera_path_by_seg_num = {
            s.name.segment_num: s.dcamera_path for s in self._segments
        }
        return [
            dcamera_path_by_seg_num.get(i, None) for i in range(self.max_seg_number + 1)
        ]

    def ecamera_paths(self):
        ecamera_path_by_seg_num = {
            s.name.segment_num: s.ecamera_path for s in self._segments
        }
        return [
            ecamera_path_by_seg_num.get(i, None) for i in range(self.max_seg_number + 1)
        ]

    def qcamera_paths(self):
        qcamera_path_by_seg_num = {
            s.name.segment_num: s.qcamera_path for s in self._segments
        }
        return [
            qcamera_path_by_seg_num.get(i, None) for i in range(self.max_seg_number + 1)
        ]
    ...
    def _get_segments_remote(self):
        ...
        for url in self.files:
            _, dongle_id, time_str, segment_num, fn = urlparse(url).path.rsplit(
                "/", maxsplit=4
            )
            ...
            if segments.get(segment_name):
                segments[segment_name] = Segment(
                    ...
                    (
                        url
                        if fn in CAMERA_FILENAMES
                        else segments[segment_name].camera_path
                    ),
                    (
                        url
                        if fn in DCAMERA_FILENAMES
                        else segments[segment_name].dcamera_path
                    ),
                    (
                        url
                        if fn in ECAMERA_FILENAMES
                        else segments[segment_name].ecamera_path
                    ),
                    (
                        url
                        if fn in QCAMERA_FILENAMES
                        else segments[segment_name].qcamera_path
                    ),
                )
            else:
                segments[segment_name] = Segment(
    ...
    def _get_segments_local(self, data_dir):
            ...
            if explorer_match:
                ...
                if segment_name.startswith(self.name.canonical_name):
                    for seg_f in os.listdir(fullpath):
                        segment_files[segment_name].append(
                            (os.path.join(fullpath, seg_f), seg_f)
                        )
                ...
                for seg_num in os.listdir(fullpath):
                    ...
                    for seg_f in os.listdir(os.path.join(fullpath, seg_num)):
                        segment_files[segment_name].append(
                            (os.path.join(fullpath, seg_num, seg_f), seg_f)
                        )
        ...
        for segment, files in segment_files.items():
            ...
            try:
                log_path = next(
                    path for path, filename in files if filename in LOG_FILENAMES
                )
            ...
            try:
                qlog_path = next(
                    path for path, filename in files if filename in QLOG_FILENAMES
                )
            except StopIteration:
                qlog_path = None
            ...
            try:
                camera_path = next(
                    path for path, filename in files if filename in CAMERA_FILENAMES
                )
            except StopIteration:
                camera_path = None
            ...
            try:
                dcamera_path = next(
                    path for path, filename in files if filename in DCAMERA_FILENAMES
                )
            except StopIteration:
                dcamera_path = None
            ...
            try:
                ecamera_path = next(
                    path for path, filename in files if filename in ECAMERA_FILENAMES
                )
            except StopIteration:
                ecamera_path = None
            ...
            try:
                qcamera_path = next(
                    path for path, filename in files if filename in QCAMERA_FILENAMES
                )
            except StopIteration:
                qcamera_path = None
        ...
        if len(segments) == 0:
            raise ValueError(
                f"Could not find segments for route {self.name.canonical_name} in data directory {data_dir}"
            )
...
class SegmentName:
    ...
    def __init__(self, name_str: str, allow_route_name=False):
        ...
        self._name_str = (
            name_str[data_dir_path_separator_index + 1 :] if use_data_dir else name_str
        )
        self._data_dir = (
            name_str[:data_dir_path_separator_index] if use_data_dir else None
        )
        ...
        self._canonical_name = (
            f"{self._route_name._dongle_id}|{self._route_name._time_str}--{self._num}"
        )
...
def get_max_seg_number_cached(sr: "SegmentRange") -> int:
    try:
        api = CommaApi(get_token())
        max_seg_number = api.get("/v1/route/" + sr.route_name.replace("/", "|"))[
            "maxqlog"
        ]
...
class SegmentRange:
    ...
    def __str__(self) -> str:
        return (
            ...
            + (f"/{self.slice}" if self.slice else "")
